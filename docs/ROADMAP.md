...
- Confirm your Cerebras API key is active. Test it with a single curl call to the API.
- Install and configure Ollama. Pull the Qwen3 model for local fallback.
...
- After installation completes: open a Python shell and import every library one by one. The list to test: fastapi, uvicorn, langgraph, neo4j, chromadb, cerebras, ollama, chronos, sklearn, shap, torch, numpy, pandas, dotenv, httpx, cryptography
...
- Cerebras: make a single test API call with a simple prompt. Confirm you get a response.
- Ollama: install and configure Ollama with Qwen3 model. Make one test call to verify local LLM is working.
...
**What to do:**
- Build `n5_reasoning.py` with Cerebras + Ollama integration
...
- Run the Cerebras API call with the output schema
...
- Test the failover: disable the Cerebras API key and confirm the call routes to Ollama
- Run the Cerebras call 10 times with the same prompt. Confirm valid JSON is returned all 10 times. If any run returns invalid JSON, update the schema enforcement in the prompt.

**Done condition:** 10/10 runs return valid JSON. Fallback confirmed working. Ollama failover confirmed working. recommended_action_id matches a real playbook.
...
**What to do:**
- Fallback 1 — LLM failure: disable the Cerebras API key mid-run. Confirm Ollama fallback activates. Re-enable Cerebras. Confirm it routes back.
