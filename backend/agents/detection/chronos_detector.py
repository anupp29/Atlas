"""
Chronos-Bolt time-series foundation model wrapper.
...
class ChronosDetector:
    ...
    Scoped per client_id + service_name. Falls back to z-score on failure.
    """
    ...
    async def score(self, values: list[float]) -> dict[str, Any]:
        """
        Score a time-series for anomaly probability.

        Args:
            values: Ordered metric readings, most recent last. Min 10 required.
        ...
        if await _ensure_model_loaded():
            return await self._chronos_score(values)
        return self._zscore_fallback(values)
    ...
    def _infer(self, values: list[float]) -> dict[str, Any] | None:
        try:
            ...
            forecast = _chronos_pipeline.predict(ctx, prediction_length=1)
            ...
            logger.warning("chronos_detector.infer_error", error=str(exc))
            return None

    def _zscore_fallback(self, values: list[float]) -> dict[str, Any]:
        # Prefer stored baseline (known-normal data) over the input window itself.
        # The input window may contain anomalous values that would inflate std and
        # suppress the z-score. Use only the first 80% of the input as history
        # when no stored baseline is available.
        baseline_key = "error_rate"
        stored = self._baseline.get(baseline_key, [])
        if len(stored) >= _MIN_POINTS:
            history = np.array(stored, dtype=float)
        else:
    ...
    def get_baseline_values(self, metric_name: str) -> list[float]:
        """Return the stored baseline readings for a metric."""
